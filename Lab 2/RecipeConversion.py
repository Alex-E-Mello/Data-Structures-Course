...
def getFile():
    ...
    while(attempts < 3 and fileNotFound == True):
        try:
            file_name = input("Please enter the file to open: ")
            input_file = open(file_name, "r")
            fileNotFound = False

        except:
            attempts = attempts + 1
            print("Attempt number: " + str(attempts) + ". You only get 3 tries, good luck!")
...
def convertLine(line, factor):
    ...
    if line[0].isdigit() == True:
        ...
        rem_line = removeMeasure(line)
        originalFraction = scanAsFraction(line)
        convertedFraction = originalFraction * factor
        if convertedFraction.getDenominator() == 1:
            conv_line = str(convertedFraction.getNumerator()) + " " + rem_line

        else:
            conv_line = str(convertedFraction) + " " + rem_line
